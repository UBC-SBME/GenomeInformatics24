---
title: BMEG 424/525 Assignment 6
output:
  github_document:
    toc: true
    toc_depth: 4
...
2.  `nextflow` is a tool for running workflows/pipelines. It is a
    ...
    **USE THE PROVIDED CONDA YML FILE FOR SIMPLICITY.**
...
3.  **Sample call rate:** Similar to SNP call rate, we retain samples
    with greater than \~98% calls. For our data we would like to *remove
    all samples with more than 2% missingness*.
...
plink --bfile /projects/bmeg/A6/MiniCohort/Mini_cohort --out /home/<user>/assignments/a6/MiniCohort/Mini_cohort_QCed
...
PCs and obscure the true data structure. Therefore, we should remove
SNPs in high LD before performing the PCA if our goal is to focus on the
entire genome, and not separate by LD-blocks.
...
#?# 20. What assumption, if any, of PCA is violated by the presence of
SNPs in LD with one another? Why? (1pt)
